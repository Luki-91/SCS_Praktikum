...
organism <- "mouse"
seuratObj <- readRDS("combined_seurat_object.rds")

#### loading ligand receptor matrix and weighted networks ####
...
seuratObj$cell_type %>% table()

DimPlot_scCustom(seuratObj, group.by = "cell_type", reduction = "umap")

lr_network <- lr_network %>% distinct(from, to)
head(lr_network)

#### Performing Nichenet analysis ####
# setting the receiver population
Idents(seuratObj) <- seuratObj$cell_type
receiver = "Memory_CD8_Trm"
expressed_genes_receiver <- get_expressed_genes(receiver, seuratObj, pct = 0.05)

# getting a list of all receptors
all_receptors <- unique(lr_network$to)
expressed_receptors <- intersect(all_receptors, expressed_genes_receiver)

potential_ligands <- lr_network %>% filter(to %in% expressed_receptors) %>% pull(from) %>% unique()

# setting the sender_celltypes
sender_celltypes <- c("classical_stroma", "special_stroma", "ST2","contamination")
...
# defining the background gene set
...
p_dotplot <- DotPlot(subset(seuratObj, cell_type %in% sender_celltypes),
...
# This will catch everything else including NAs
